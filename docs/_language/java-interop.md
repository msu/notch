...
description: "Call JVM methods, construct objects, read properties, resolve types, and iterate enums from Notch, where the JVM is first-class."
order: 8
---

Notch is designed to live inside the Java ecosystem. JVM classes, methods, and
properties are first-class values in Notch code.
...
When more than one overload of a method exists, Notch picks the best match by
parameter count, then by direct argument-type assignability, then by whether a
registered coercion can bridge the argument type to the parameter type.
Less-specific matches lose to more-specific ones.
...
[`is empty`](../operators/#emptiness) or compare against `null` explicitly to
guard.

## Property access

Property access works on JVM objects via their getter methods. Both bean-style and
direct field names are accepted.
...
- [Type Coercions](../coercions/) for how Notch lets `int` flow into `long`-typed parameters.
- [Functions & Closures](../functions/) for converting Notch closures into Java functional interfaces.
- [Errors & Exceptions](../errors/) for throwing and catching JVM exceptions.
